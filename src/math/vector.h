...
struct Vec2 {
  ...
  static inline constexpr Vec2 ZERO() { return Vec2{.x = 0, .y = 0}; }

  static inline constexpr Vec2 ONE() { return Vec2{.x = 1, .y = 1}; }

  inline Vec2 operator+(const f32 s) const { return {x + s, y + s}; }
  ...
  inline constexpr Vec2 operator-(const f32 s) const { return {x - s, y - s}; }
