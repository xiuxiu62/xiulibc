...
PoolStatus pool_create(Pool &pool, usize stride, usize initial_capacity) {
  ...
  pool.slots =
      static_cast<usize *>(calloc(initial_capacity * sizeof(usize), 0));
  pool.validity_bitmap =
      static_cast<u8 *>(calloc((initial_capacity + 7) / 8, 0));
...
PoolStatus pool_reserve(Pool &pool, usize capacity) {
  ...
  usize *slots =
      static_cast<usize *>(realloc(pool.slots, capacity * sizeof(usize)));
  u8 *validity_bitmap = static_cast<u8 *>(
      realloc(pool.validity_bitmap, capacity * sizeof(usize)));

  if (!data || !slots || !validity_bitmap)
    return PoolStatus::RESIZE_FAILED;

  memset(slots + pool.capacity, 0, (capacity - pool.capacity) * sizeof(usize));
  memset(validity_bitmap + ((pool.capacity + 7) / 8), 0,
         ((capacity + 7) / 8) - ((pool.capacity + 7) / 8));
...
PoolStatus pool_shrink_to_fit(Pool &pool) {
  if (pool.extent == 0)
    return PoolStatus::UNCHANGED;

  // Round to next power of 2
  usize min_capacity = pool.extent - 1;
  ...
  if (pool.capacity - min_capacity < (pool.capacity / 4))
    return PoolStatus::UNCHANGED; // Less than 25% waste

  u8 *data = static_cast<u8 *>(realloc(pool.data, min_capacity * pool.stride));
  usize *slots =
      static_cast<usize *>(realloc(pool.slots, min_capacity * sizeof(usize)));
  u8 *validity_bitmap =
      static_cast<u8 *>(realloc(pool.validity_bitmap, (min_capacity + 7) / 8));
